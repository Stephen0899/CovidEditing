...
class TransformGraph():
	...
	def get_reg_preds(self, logit):
		...
		# Xray
		preds, _ = self.regressor(logit)
		preds = torch.stack(preds).squeeze()
		# [4, 5]
		preds = torch.transpose(preds, 0, 1)
		preds = torch.sigmoid(preds[:, self.attrIdx])

		# preds = torch.sigmoid(self.regressor(logit)[:, self.attrIdx])
	...
	def get_reg_loss(self, feed_dict):
		...
		preds = self.get_reg_preds(logit)
		print('in get_reg_loss', preds.size(), preds, alpha_gt)
	...
	def get_reg_module(self):
		...
		# Xray
		if constants.reg_json is not None:
			with open(constants.reg_json) as f:
				cfg = edict(json.load(f))
			from  ..xray_classifier.classifier import Classifier  # noqa
	...
	def get_vgg_module(self):
		...
		vgg19.load_state_dict(torch.load('vgg19.ckpt'))
		vgg19 = vgg19.cuda().module.features.eval()
		return vgg19
			...
			if self.stylegan_opts.latent == 'w':
				...
				if index_!=None:
					...
					if len(self.attrIdx) == len(self.attrTable):
						# print('alpha_delta[:, index_]', alpha_delta[:, index_], alpha_to_graph)
	...
	def vis_multi_image_batch_alphas(self, graph_inputs, filename,
		...
		# img_org = np.uint8(np.clip(((img_org + 1) / 2.0) * 255, 0, 255))
